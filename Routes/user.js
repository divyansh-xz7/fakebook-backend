...
router.get('/:ID/profile', auth, async (req,res)=>{
    try {
        ...
        if (isEmail)
        var user = await User.findOne({email:ID},{password:0});
        else
        ...
        let result;
        if (isFollowing)
        result = {_id:user._id,name:user.name, email:user.email, profilePicture:user.profilePicture, desc:user.desc, following:user.following, followers:user.followers, personalInfo:user?.personalInfo, follow:"yes"};
        else if(isMe)
        result = {_id:user._id,name:user.name, email:user.email, profilePicture:user.profilePicture, desc:user.desc, following:user.following, followers:user.followers, personalInfo:user?.personalInfo, requests:user?.followRequests, requestedToFollow:user?.requestedToFollow, follow:"yes", chat:user?.chat, messages:user?.messages};
        else
        result = {_id:user._id,name:user.name, email:user.email, profilePicture:user.profilePicture, desc:user.desc, followers:user.followers.length, following:user.following.length, follow:"no"};
...
router.get('/:id/follow', auth, async (req,res)=>{
    try {
        ...
        const user = await User.findById(id,{followers:1,following:1,followRequests:1,requestedToFollow:1});

        if(!user) return res.status(404).json({message:"User doesn't exists!"});
        if(user.followers.includes(userId)) return res.status(200).json({message:"You already follow this user!"});
        if(user.followRequests.includes(userId)) return res.status(200).json({message:"Requested to follow the user, wait until they accept your request!"});

        const thisUser = await User.findById(userId, {followers:1,following:1,followRequests:1,requestedToFollow:1});
...
router.get('/:id/unfollow', auth, async (req,res)=>{
    try {
        ...
        if(!(user.followers.includes(userId) || user.followRequests.includes(userId))) return res.status(200).json({message:"You don't follow this user!"});
        ...
        thisUser.requestedToFollow=thisUser.requestedToFollow.filter((u)=>u!==id);
...
router.get('/:id/acceptfollow',auth, async (req,res)=>{
    try {
        ...
        const user = await User.findById(id,{following:1,requestedToFollow:1});

        if(!user) return res.status(200).json({message:"User doesn't exists!"});
        
        if(!user.requestedToFollow.includes(userId)) return res.status(200).json({message:"User has not requested to follow you!"});

        const thisUser = await User.findById(userId,{followers:1,followRequests:1});
...
router.post('/chats', auth, async(req,res)=>{
    try {
        ...
        if(user.chat.includes(newChat)) return res.status(200).json({message:"chat already in list"});
        const otherUser = await User.findById(newChat,{chat:1});
        
        if(!otherUser.chat.includes(userId))
        otherUser.chat.unshift(userId);
...
router.post('/chats/messages', auth, async(req,res)=>{
    try {
        ...
        res.status(200).json({res:newMess, message:"Message Posted Successfully!"});
...
router.delete('/chats/:chatId/messages/:messageId', auth, async(req,res)=>{
    try {
        ...
        const user  = await User.findByIdAndUpdate(userId,{$pull:{messages:{"_id":messageId}}},{messages:1});
        const other = await User.findByIdAndUpdate(chatId,{$pull:{messages:{"_id":messageId}}},{messages:1});
...
router.get('/searchuser/:username/:exact', auth, async(req, res)=>{
    try {
        ...
        if(exact=='0')
        {
            let users = await User.find({email: new RegExp('^' + username+'.*')}, {profilePicture:1, name:1, email:1}).limit(20);
            res.status(200).json(users);
        }
        else
        {
            let users = await User.find({email:username}, {profilePicture:1, name:1, email:1});
            res.status(200).json(users);
        }
    } catch (error) {
        res.status(501).json(error);
